		...
		unsigned int buflength)
{
	...
	uvc_trace(UVC_TRACE_VIDEO, "Allocated uvc buffer, n = %d, size = %d\n", nbuffers, bufsize);
done:
	mutex_unlock(&queue->mutex);
	return ret;
}
...
int uvc_queue_mmap(struct uvc_video_queue *queue, struct vm_area_struct *vma)
{
	...
	/* Already maped, vma is not belong to this queue
	 *
	 * I almost sure that this will never happend, because vm_pgoff
	 * check is enought.
	 */
...
/* Map memory based on the size
 *
 * uvc_queue_mmap() already does the sanity check to
 * make sure map to correct queue memory base.
 */
int uvc_video_mmap(struct uvc_streaming *stream, struct vm_area_struct *vma)
{
	int ret;

	uvc_printk(KERN_INFO, "mmapping\n");
	...
	uvc_printk(KERN_ERR, "Failed to mmap vma: %p\n", vma);
	return ret;
}
